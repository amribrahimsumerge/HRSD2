...
public class BasePage {
    ...
    public BasePage(WebDriver driver) {
        ...
        this.fluentWait = new FluentWait<WebDriver>(driver)
                ...
                .pollingEvery(Duration.ofSeconds(1)) // Adjusted polling
    ...
    public WebElement getShadowRootElement(ArrayList<String> cssSelectors) throws InterruptedException {
        ...
        while (attempts < 3) {
            try {
                WebElement root = driver.findElement(By.cssSelector(cssSelectors.get(0)));
                SearchContext shadow = root.getShadowRoot();

                ArrayList<SearchContext> shadows = new ArrayList<>();
                shadows.add(shadow);

                // Navigate down the nested shadows if needed
                for (int i = 0; i < cssSelectors.size() - 2; i++) {
                    WebElement nestedHost = shadows.get(i).findElement(By.cssSelector(cssSelectors.get(i + 1)));
                    shadows.add(nestedHost.getShadowRoot());
                }

                // Return the final target element
                return shadows.get(shadows.size() - 1)
    ...
    public WebElement locateElementsInShadowRoot(String Host, String Target) throws InterruptedException {
        int attempts = 0;
        while (attempts < 3) {
            try {
        ...
        throw new NoSuchElementException("Shadow root element not found after 3 attempts: Host=" + Host + ", Target=" + Target);
    }

    // Amr Ibrahim: NEWLY ADDED METHOD
    protected void fillInputInShadow(String hostSelector, String targetSelector, String value) throws InterruptedException {
